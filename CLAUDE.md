...
### 実装前のステートマシン図
イベント駆動・状態依存のロジック（マウス/D&D・編集モード・非同期ロード・Dirty 検知・
ウィザード遷移など、入力の順序や内部フラグで挙動が変わるもの）を新規実装・修正する前に、
**ステートマシン図（状態・遷移・各遷移のガード条件）を必ず作成**し、それを用いて
クラッシュ・デッドロック・状態残留などの危険状態が**到達不能であること**を確認してから
実装に着手すること。図は回答内に Mermaid（`stateDiagram-v2`）で示し、要点はタスクの
`claudeNote` に残す。単純な CRUD やステートレスな処理には不要。
